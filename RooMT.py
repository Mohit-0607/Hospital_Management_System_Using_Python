...
def room_button():
    ...
    tkinter.messagebox.showinfo("Dr. GULATI DATABASE SYSTEM", "ROOM ALLOCATED")
    conn.commit()
    conn.close()
...
def update_button():
    ...
    if len(p) != 0:
        ...
        tkinter.messagebox.showinfo("Dr. GULATI DATABASE SYSTEM", "ROOM DETAILS UPDATED")
        conn.commit()
    else:
        tkinter.messagebox.showinfo("Dr. GULATI DATABASE SYSTEM", "PATIENT IS NOT ALLOCATED A ROOM")
...
def ROOMD_button():
    ...
    if (len(p)==0):
        tkinter.messagebox.showinfo("Dr. GULATI DATABASE SYSTEM","PATIENT NOT ALLOCATED ROOM")
    else:
